...
// copy fonts files from src to dist
gulp.task('copy_fonts', function() {
  return gulp.src(SRC_DIR + 'fonts/*')
    .pipe(gulp.dest(DIST_DIR + 'fonts'))
    .pipe(gulpsize({ title : 'fonts' }))
    .pipe(gulpif(!argv.pub, browserSync.stream()))  // 开发模式下，自动刷新
    .pipe(notify({ message: 'copy_fonts task ok' }));
});
...
gulp.task('browser-sync', function () {
    ...
    var proxy = proxyMiddleware(['/api', '/query','/appindex'], {target: 'http://10.2.122.58:93'/*, pathRewrite: {'^/api' : ''}*/});
    // var proxy1 = proxyMiddleware('/blacklist', {target: '10.2.122.58:91'});
    //http://10.2.122.58:91/appindex/api/navs
    var proxy1 = proxyMiddleware('/blacklist', {target: 'http://10.2.130.200:8090'});
    browserSync.init({
        server: {
            baseDir: DIST_DIR,
            middleware: [/*historyApiFallback(), */proxy, proxy1],
...
gulp.task('default', function (cb) {
  runSequence('clean', 'img', 'html', 'test_data', 'js_build', 'scss', 'copy_fonts', 'browser-sync', 'watch', function () {
      cb && cb();
  })
});
// waits until clean is finished then builds the project
gulp.task('build', ['clean'], function(){
  gulp.start(['img', 'html','js_build','scss','copy_fonts']);
});
